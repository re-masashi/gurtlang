...
use crate::typechecker::{TypeEnv, StructTy};
use crate::{t_bool, t_float, t_int, t_list, t_string, tvar, t_unit};

impl TypeEnv<'_> {
    pub fn struct_to_typed_struct(&mut self, struct_: (&Struct, &Range<usize>)) -> (TypedStruct, Range<usize>) {
        let (struct_, _span) = struct_;
        let Struct { name, generics, fields } = struct_;
        ...
        let typed_fields = fields
            ...
            .map(|(field_name, type_annot, range)| {
                let ty = self.type_annot_to_type(type_annot);
